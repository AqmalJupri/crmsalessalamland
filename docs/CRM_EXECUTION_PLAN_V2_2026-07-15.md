...
The matrix below preserves the 15 July 2026 `codex/production-foundation` baseline. It is historical, not the present Round 1 checkpoint. Round 1 now checksum-locks `0001`–`0007`, implements the source-neutral migration/import/reconciliation control plane, and has local and hosted synthetic evidence for migration replay, dump/restore identity, coverage, database invariants, immutable dual-surface identity and reviewed Linux visual baselines. Green candidate run `29660323321` was rejected after log audit found five Playwright default-caret hydration warnings. Corrected forced-fresh run `29660877155` completed cleanly, its 60 independently reviewed assets were promoted in `2ed7379`, and standard reviewed-baseline run `29661378588` passed every job with zero real hydration/caret diagnostic, flaky test, retry or screenshot mismatch. Signed registry-image promotion and every production deployment approval remain open. “Authored” or “implemented” is not synonymous with production-ready, approved or released. All gate states remain **Not passed**.
...
| Toolchain and CI | New release work pins Ubuntu 24.04 x64, exact hosted/build Node.js `22.23.1` within package compatibility `<23`, pnpm `11.9.0`, a separately digest-locked Distroless Node 22 Debian 13 runtime, checksum-locked release tools, an exact action repository/SHA allowlist, and the PostgreSQL 16.14 image digest; lint/typecheck/two-layer coverage/build/runtime/DB/E2E plus dual-image/SBOM/scan/manifest/restricted-smoke/attestation gates are authored | Current local candidate: 8 release/CI files and 213/213 pass; capture-mode high-confidence 1,110 pass/1 intentional provenance skip; production 1,367/1,367; database 601/601; both Next surface builds and dependency audit pass | Historical hosted runs `29660877155` and `29661378588` remain green on their reviewed source. The new Node `22.23.1` exact OCI path still requires a fully green hosted run and reviewed Linux provenance refresh. No registry promotion or production deployment evidence exists | **Not passed** |
...
- Orders, ledger, payment/refund/reconciliation, all three vertical modules, provider connectors, conversations and server KPI/report/export are not implemented. The source-neutral import/reconciliation control plane exists, but all four real-source adapters, approved mappings, final extracts, reconciliation plans and cutover tooling remain absent.
- Clean forced-fresh combined capture run `29660877155` and standard comparison run `29661378588` are fully green, and reviewed visual provenance is committed in `2ed7379`; the rejected `29660323321` candidate remains recorded as evidence that green workflow status alone is insufficient when logs contain hydration defects. No signed registry image, trusted deployment promotion, infrastructure-as-code, production monitoring or recovery automation is evidenced.
