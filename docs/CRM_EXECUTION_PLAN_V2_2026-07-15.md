...
The matrix below preserves the 15 July 2026 `codex/production-foundation` baseline. It is historical, not the present Round 1 checkpoint. Round 1 now checksum-locks `0001`–`0007`, implements the source-neutral migration/import/reconciliation control plane, and has local evidence for migration replay, dump/restore identity, coverage and database invariants. Hosted runs have exercised the pinned toolchain and failed closed on stale visual provenance; they have not yet completed the combined pipeline. Final combined-source visual provenance, fully green hosted Quality, immutable artifact promotion and every production deployment approval remain open. “Authored” or “implemented” is not synonymous with production-ready, approved or released. All gate states remain **Not passed**.
...
| Toolchain and CI | Runtime is pinned to Node.js `22.22.0`/pnpm `11.9.0`; checkout `34e114…f8d5`, pnpm setup `b906af…f58b1`, Node setup `49933e…0020`, and the PostgreSQL 16.14 image digest are exact; lint/typecheck/two-layer coverage/build/runtime/DB/E2E jobs are authored | Historical baseline: **220 tests across 34 unit/component files** passed. Round 1 evidence and current counts are maintained in `.superpowers/sdd/progress.md` and the PRD evidence ledger | The workflow has executed remotely and failed closed where expected, but no final green combined-source run, signed immutable artifact or promotion evidence exists; package audit is not full security review | **Not passed** |
...
- Hosted CI has executed and failed closed on stale visual provenance, but no final green combined-source run, signed immutable artifact, deployment promotion, infrastructure-as-code, production monitoring or recovery automation is evidenced.
