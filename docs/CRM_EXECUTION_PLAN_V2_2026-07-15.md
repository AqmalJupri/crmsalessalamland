...
The matrix below preserves the 15 July 2026 `codex/production-foundation` baseline. It is historical, not the present Round 1 checkpoint. Round 1 now checksum-locks `0001`–`0007`, implements the source-neutral migration/import/reconciliation control plane, and has local and hosted synthetic evidence for migration replay, dump/restore identity, coverage, database invariants, immutable dual-surface identity and reviewed Linux visual baselines. Forced-fresh hosted run `29657949035` completed the combined capture pipeline; standard reviewed-baseline run `29658551712` passed every job but exposed one retry-only test-hydration race, so a clean rerun remains required. Signed registry-image promotion and every production deployment approval remain open. “Authored” or “implemented” is not synonymous with production-ready, approved or released. All gate states remain **Not passed**.
...
| Toolchain and CI | Runtime is pinned to Node.js `22.22.0`/pnpm `11.9.0`; checkout `34e114…f8d5`, pnpm setup `b906af…f58b1`, Node setup `49933e…0020`, and the PostgreSQL 16.14 image digest are exact; lint/typecheck/two-layer coverage/build/runtime/DB/E2E jobs are authored | Historical baseline: **220 tests across 34 unit/component files** passed. Round 1 evidence and current counts are maintained in `.superpowers/sdd/progress.md` and the PRD evidence ledger | Forced-fresh hosted run `29657949035` is green and its 60 reviewed Linux assets are promoted in `9f2c8cd`; standard run `29658551712` passed every job but recorded one retry-only pre-hydration test mutation. A zero-flaky rerun remains required. No signed immutable registry artifact or promotion evidence exists; package audit is not full security review | **Not passed** |
...
- Hosted forced-fresh combined capture run `29657949035` is green and reviewed visual provenance is committed; standard run `29658551712` passed all jobs but disclosed one retry-only E2E test-hydration race, now corrected locally and pending a clean hosted rerun. No signed registry image, trusted deployment promotion, infrastructure-as-code, production monitoring or recovery automation is evidenced.
