...
|---|---|---|
| Lint and TypeScript | PASS on integrated Round 1 | Local shell currently uses Node 22.22.3; hosted forced-fresh capture run `29660877155` and standard reviewed-baseline run `29661378588` exercised exact application Node 22.22.0 and finished every job successfully |
| High-confidence coverage | Normal reviewed-evidence mode: 76 files, 937/937 pass. Explicit capture mode: 76 files, 936 pass and one provenance assertion skipped. Both report 92.27% statements, 85.98% branches, 95.23% functions, 93.32% lines | The skip is restricted to generating a new candidate; reviewed normal mode verifies the promoted 60-asset provenance |
