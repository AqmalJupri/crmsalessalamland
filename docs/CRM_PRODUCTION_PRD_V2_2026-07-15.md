...
The present V2 branch is a **hardened production foundation**, not a production release. Local and hosted synthetic engineering evidence now covers database, Lead-command, identity-session, migration, CSP/runtime, dual-surface artifact isolation and reviewed demo-UI baselines, but it does not satisfy any release gate. A signed and registry-promoted immutable release image, a real managed IdP and MFA policy, dedicated production infrastructure, workers/queue dispatch, private object storage, telemetry, backup/PITR and restore rehearsal, capacity/soak proof, real-source migration/UAT evidence, and named business, security, privacy, finance and operations approvals are still absent.
...
The runtime artifact pins exact hosted/build Node.js `22.23.1` within package compatibility `<23`, proves the immutable Distroless Node 22 Debian 13 runtime index/platform/config/layer ancestry, runs under numeric non-root UID/GID `65532:65532` with dropped capabilities and bounded resources, and is promoted unchanged from staging to production. The edge must prevent direct-origin bypass and document Tunnel/proxied-DNS choice, trusted proxies, origin authentication, route limits, 429 handling, and connect/read/send timeouts.
...
The reviewed migration manifest locks the exact filename, byte length and SHA-256 sequence for `0001` through `0007`. Local execution proves transactional apply, exact no-op replay with unchanged timestamped ledger, tamper rejection, isolated dump/restore ledger identity, and readiness rejection on ledger drift. Hosted forced-fresh capture run [`29660877155`](https://github.com/AqmalJupri/crmsalessalamland/actions/runs/29660877155) repeated the seven-row apply/replay and ledger-preservation path successfully on source `c0a8d90`, emitted zero real hydration/caret diagnostics, flaky tests or retries, and produced the reviewed evidence promoted in `2ed7379`. Standard reviewed-baseline run [`29661378588`](https://github.com/AqmalJupri/crmsalessalamland/actions/runs/29661378588) then passed every job with unit 937/937, production 1,367/1,367, database 601/601, CRM browser 200 passed/38 skipped and Tasha browser 113 passed/125 skipped, with zero hydration diagnostic, caret mutation, flaky test, retry or screenshot mismatch. Migration Task 11 engineering closure is complete. The synthetic source-neutral control plane covers source registration, staging, quarantine, approval, canonical apply, lineage, reconciliation and signed proof. This is not evidence that any real source adapter, mapping, final extract, business reconciliation or production cutover has passed.

Each surface build embeds an immutable `CRM_BUILD_SURFACE` marker and is produced without runtime secrets. Static metadata, manifests and safe error surfaces retain that compiled identity, while authenticated/runtime authority still requires the full validated runtime configuration. A conflicting runtime `PRODUCT_SURFACE` fails closed: readiness returns `503`, login returns `500`, and the manifest continues to identify the compiled artifact. Hosted runs `29660877155` and `29661378588` prove the positive CRM/Tasha paths and opposite-surface rejection for both artifacts; this is tar-artifact isolation evidence, not a signed registry image or deployment promotion.
...
|---|---|---|
| Lint and TypeScript | PASS on the current production-artifact candidate | Local shell uses compatible Node 22.22.3; the new workflow requires exact Node 22.23.1 on pinned Ubuntu 24.04 x64, but that path still needs its own hosted run. Earlier hosted runs remain historical Node 22.22.0 evidence |
| High-confidence coverage | Current capture mode: 82 files, 1,110 pass and one provenance assertion skipped; 92.27% statements, 85.98% branches, 95.23% functions, 93.32% lines. Prior promoted-evidence normal mode: 76 files, 937/937 pass | The skip is restricted to producing a candidate for review. Normal mode for the new source SHA cannot pass until hosted Linux evidence is reviewed and promoted |
| All-production-source coverage | 76 files, 1,367/1,367 pass; 82.63% statements, 78.05% branches, 90.46% functions, 83.94% lines | Coverage is engineering evidence, not UAT, capacity, penetration, privacy or production approval |
...
| Dual-surface runtime and browser | CRM and Tasha secret-free production builds plus runtime/security/readiness and opposite-surface rejection pass. Full local browser evidence: CRM 165 pass/73 intentionally inapplicable skip; Tasha 88 pass/143 intentionally inapplicable skip | These are synthetic foundation paths; real-provider, production database-backed workflow and business UAT evidence remain absent |
| Reviewed visual evidence | Hosted capture: CRM 35 pass/14 skip and Tasha 25 pass/10 skip; 60 synthetic Linux PNGs in artifact `8434337880`, digest `sha256:d8c4b660392bf53befc8e6546a78db6dd2d964d62b3fc491567e366483a425e2`; all assets/provenance were structurally verified and independently reviewed, with 55 byte-identical and five antialias-only differences promoted in `2ed7379` | Standard comparison `29661378588` passed with zero hydration diagnostic, caret mutation, flaky test, retry or screenshot mismatch. Named human screen-reader journey sign-off remains open; visual review is not accessibility or UAT approval |
| Production artifact candidate | 8 release/CI contract files, 213/213 pass; both CRM/Tasha Next production builds pass; dependency audit has no known moderate-or-higher vulnerability | Exact dual OCI image build, SBOM/scan/manifest output, restricted downloaded-image smoke, attestation and artifact digests remain unproven until hosted CI completes. No registry promotion or deployment is authorized |
...
Reproducibility for new release-image work is authored around exact hosted/build Node.js `22.23.1` within package compatibility `>=22.22.0 <23`, a separately digest-locked Distroless Node 22 Debian 13 runtime, pnpm `11.9.0`, Next.js `16.2.10`, React `19.2.7`, TypeScript `6.0.3`; commit-pinned checkout `34e114876b0b11c390a56381ad16ebd13914f8d5`, pnpm setup `b906affcce14559ad1aafd4ab0e942779e9f58b1`, and Node setup `49933ea5288caeca8642d1e84afbd3f7d6820020`; and PostgreSQL image `postgres:16.14-alpine@sha256:57c72fd2a128e416c7fcc499958864df5301e940bca0a56f58fddf30ffc07777`. Earlier Round 1 hosted runs correctly remain historical Node `22.22.0` evidence. Candidate run `29660323321` was rejected despite a green workflow because five Playwright default-caret hydration warnings remained. Corrected forced-fresh run `29660877155` is clean, its reviewed Linux evidence is promoted in `2ed7379`, and standard run `29661378588` passed with zero real hydration/caret diagnostic, flaky test, retry or screenshot mismatch. No signed immutable registry image, trusted promotion, staging/production deployment or production release exists. Earlier runs failed closed as designed on stale or rejected visual evidence rather than accepting it to make CI green.
...
- edge/origin isolation, immutable Distroless Node 22 runtime with exact Node `22.23.1` evidence and numeric non-root UID/GID `65532:65532`, resource/health limits, database-role/RLS design and approved queue failure boundary are evidenced.
