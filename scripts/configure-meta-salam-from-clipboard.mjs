...
if (!rawMetaToken) {
  throw new Error("No Meta token found. Copy the Page access token first or set CRM_META_TOKEN.");
}
...
async function resolvePageToken(token) {
  try {
    await readGraph(`${pageId}/leadgen_forms`, token, {
      fields: "id,name,status",
      limit: "1"
    });
    return { token, source: "page_token" };
  } catch (error) {
  ...
  if (!page?.access_token) {
    throw new Error(`Could not derive Page Access Token for page ${pageId}. Check Page access and permissions.`);
  }
...
const resolvedMetaToken = await resolvePageToken(rawMetaToken);
const metaToken = resolvedMetaToken.token;
...
const nextConnections = (current.connections || []).map((connection) => {
  ...
  return {
    ...
    metaSpendAccessToken: metaToken,
...
console.log(JSON.stringify({
  ...
  tokenSource: resolvedMetaToken.source,
  salamLand: {
